...
class TripletMNIST_MT(Dataset):
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            label2 = self.train_labels[positive_index].item()
            label3 = self.train_labels[negative_index].item()
        else:
            img1 = self.test_data[self.test_triplets[index][0]]
            img2 = self.test_data[self.test_triplets[index][1]]
            img3 = self.test_data[self.test_triplets[index][2]]
            
            label1 = self.test_labels[index].item()
            label2 = self.test_labels[positive_index].item()
            label3 = self.test_labels[negative_index].item()
        ...
        return (img1, img2, img3), target, (label1, label2, label3)
...
class BalancedBatchSampler_Class(BatchSampler):
    """
    BatchSampler - from a MNIST-like dataset, samples n_classes and within these classes samples n_samples.
    Returns batches of size n_classes * n_samples
    """
